...
class PoseController:
    ...
    def __init__(self, esp32_host="192.168.4.1", esp32_port=8000):
        """
    ...
    def connect(self):
        ...
        Automatically tries port 8000 first, then falls back to 8080 if needed
        
        Returns:
            bool: True if connected, False otherwise
        """
        # Try both ports: 8000 (new JSON) and 8080 (old binary)
        ports_to_try = [self.esp32_port]
        if self.esp32_port == 8000:
            ports_to_try.append(8080)  # Fallback to old port
        
        last_error = None
        
        for port in ports_to_try:
            try:
                print(f"Attempting to connect to ESP32 at {self.esp32_host}:{port}...")
                
                self.socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                self.socket.settimeout(5)  # 5 second timeout
                self.socket.connect((self.esp32_host, port))
                self.connected = True
                self.esp32_port = port  # Update port if we connected to a different one
                
                if port == 8080:
                    print(f"✓ Connected to ESP32 on port {port} (OLD firmware detected)")
                    print(f"  ⚠ WARNING: You're using old firmware (port 8080, binary protocol)")
                    print(f"  → Upload the updated firmware to use JSON protocol on port 8000")
                else:
                ...
                if port != ports_to_try[-1]:  # Not the last port
                    print(f"  Port {port} timeout, trying next port...")
                    continue
            except ConnectionRefusedError:
                last_error = f"Connection refused on port {port}"
                if port != ports_to_try[-1]:  # Not the last port
                    print(f"  Port {port} refused, trying port {ports_to_try[-1]}...")
                    continue
            except socket.gaierror as e:
                print(f"✗ DNS/Hostname resolution failed: {e}")
                print(f"  → Check if {self.esp32_host} is the correct IP address")
                print(f"  → Verify WiFi connection to ESP32_AP")
                self.connected = False
                return False
            except socket.error as e:
                err_code = getattr(e, 'winerror', getattr(e, 'errno', None))
                last_error = f"Socket error {err_code} on port {port}"
                if port != ports_to_try[-1]:  # Not the last port
                    print(f"  Port {port} failed (error {err_code}), trying next port...")
                    continue
            except Exception as e:
                last_error = f"Unexpected error on port {port}: {e}"
                if port != ports_to_try[-1]:  # Not the last port
                    continue
        
        # If we get here, all ports failed
        print(f"✗ Failed to connect to ESP32 on any port")
        print(f"  Last error: {last_error}")
        print(f"  Tried ports: {', '.join(map(str, ports_to_try))}")
        print(f"  Troubleshooting:")
        print(f"  1. Verify you're connected to 'ESP32_AP' WiFi network")
        print(f"  2. Check ESP32 Serial Monitor (115200 baud) for server status")
        print(f"  3. Upload the updated firmware to ESP32")
        print(f"  4. Look for 'TCP server started on port XXXX' in Serial Monitor")
        print(f"  5. Run 'python roboCleaner/client/find_esp32_port.py' to detect which port is open")
        self.connected = False
        return False
